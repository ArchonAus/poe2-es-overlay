...
GAME_WINDOW_ID = None
CAPTURE_PATH = "/tmp/poe2_es_overlay_capture.png"
...
def find_game_window_id():
    try:
        result = subprocess.check_output(
            ["xdotool", "search", "--onlyvisible", "--name", GAME_WINDOW_NAME],
            text=True,
        ).strip().splitlines()
        if not result:
            return None
...
def capture_ocr_region(win_x, win_y, win_w, win_h):
    ...
    subprocess.run(["scrot", "-o", "-a", f"{x},{y},{w},{h}", CAPTURE_PATH], check=True)
...
class Overlay(QWidget):
    ...
    def tick(self):
        try:
            ...
            img = capture_ocr_region(win_x, win_y, win_w, win_h)
